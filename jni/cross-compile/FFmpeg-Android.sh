...
if [ -d android/ffmpeg ]; then
  ...
  cd android/ffmpeg
  patch -p1 < ../../patch/ffmpeg/ffmpeg-0.8.14.patch
fi
...
for version in neon armv7 vfp armv6; do
  ...
  $CC -lm -lz -shared --sysroot=$SYSROOT -Wl,--no-undefined -Wl,-z,noexecstack $EXTRA_LDFLAGS libavutil/*.o libavutil/arm/*.o libavcodec/*.o libavcodec/arm/*.o libavformat/*.o libswscale/*.o -o $PREFIX/libffmpeg.so
